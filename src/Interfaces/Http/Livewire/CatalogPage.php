...
class CatalogPage extends Component
{
    ...
    public function render(CatalogProductListQuery $products, CatalogFiltersQuery $filters): View
    {
        ...
        $categoryScopeIds = $this->resolveScopedCategoryIds($currentCategory);

        if ($this->slug !== null && $currentCategory === null) {
            return view('shophats::pages.home', ['title' => 'Catalog: '.$this->slug]);
        }

        $categoryFilters = $this->filters;

        if ($currentCategory !== null) {
            $selectedCategories = collect((array) ($categoryFilters['categories'] ?? []))
                ->map(fn ($value) => is_numeric($value) ? (int) $value : (string) $value)
                ->intersect($categoryScopeIds)
                ->values()
                ->all();
        ...
        $categories = $currentCategory !== null
            ? $currentCategory->children
                ->sortBy(fn (Category $category) => $category->translation($this->locale)?->name ?? $category->path)
                ->values()
            : Category::query()
                ->whereNull('parent_id')
        ...
        $seo = $currentCategory
            ? app(LocalizedSeoResolver::class)->forTranslatedContent(
                locale: $this->locale,
        ...
        return view('shophats::pages.catalog', [
            ...
            'sidebarContext' => [
                'category_scope' => $categoryScopeIds,
            ],
    ...
    private function resolveCurrentCategory(): ?Category
    {
        if (! filled($this->slug)) {
            return null;
        }
        ...
        $this->loadAncestorChain($category);
    ...
    {
        return app(CatalogFiltersQuery::class)->handle(
            $this->locale,
            $this->resolveScopedCategoryIds($this->resolveCurrentCategory())
        )['price'] ?? [
            'min' => 0,
            'max' => 1000,
        ];
    ...
    private function resolveScopedCategoryIds(?Category $category): array
    {
        if (! $category) {
            return [];
        }
    ...
    private function resolveCategoryBlocks(?Category $category): array
    {
        if (! $category) {
            return [];
        }

        $translation = $this->resolveRenderableCategoryTranslation($category);

        if ($translation && ! empty($translation->blocks ?? [])) {
            return array_values($translation->blocks);
        }

        return [];
    }

    private function resolveRenderableCategoryTranslation(Category $category): ?object
    {
        $translation = $category->translations->firstWhere('locale', $this->locale);

        if ($this->hasRenderableCategoryContent($translation)) {
            return $translation;
        }

        return $category->translations->firstWhere('locale', Locales::default());
    }

    private function hasRenderableCategoryContent(?object $translation): bool
    {
        if (! $translation) {
            return false;
        }

        return ! empty($translation->blocks ?? []);
    }

    private function loadAncestorChain(Category $category): void
    {
        $currentCategory = $category;

        while ($currentCategory->parent_id !== null) {
            $currentCategory->loadMissing([
                'parent' => fn ($query) => $query->with('translations'),
            ]);
