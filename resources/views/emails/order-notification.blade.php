...
<div style="max-width:680px;margin:0 auto;padding:32px 16px">
    <div style="background:#fff;border:1px solid #e5e5e5;padding:28px">
        ...
        <table style="width:100%;border-collapse:collapse;margin:0 0 24px">
            <tr><td style="padding:7px 0;color:#666">{{ __('commero::app.order_notifications.order_number') }}</td><td style="padding:7px 0;text-align:right;font-weight:700">{{ $order->number }}</td></tr>
            <tr><td style="padding:7px 0;color:#666">{{ __('commero::app.order_notifications.customer_name') }}</td><td style="padding:7px 0;text-align:right">{{ $order->customer_name }}</td></tr>
            @if(filled($order->customer_email))
                <tr><td style="padding:7px 0;color:#666">{{ __('commero::app.order_notifications.customer_email') }}</td><td style="padding:7px 0;text-align:right">{{ $order->customer_email }}</td></tr>
            @endif
            ...
            @endif
            <tr><td style="padding:7px 0;color:#666">{{ __('commero::app.order_notifications.status') }}</td><td style="padding:7px 0;text-align:right;font-weight:700">{{ $statusLabel }}</td></tr>
        </table>

        <h2 style="font-size:18px;margin:0 0 10px">{{ __('commero::app.order_notifications.items') }}</h2>
        <table style="width:100%;border-collapse:collapse;border-top:1px solid #ddd">
            <thead><tr>
                <th style="padding:10px 4px;text-align:left;border-bottom:1px solid #ddd">{{ __('commero::app.order_notifications.product') }}</th>
                <th style="padding:10px 4px;text-align:right;border-bottom:1px solid #ddd">{{ __('commero::app.order_notifications.quantity') }}</th>
                <th style="padding:10px 4px;text-align:right;border-bottom:1px solid #ddd">{{ __('commero::app.order_notifications.total_amount') }}</th>
            </tr></thead>
            <tbody>
            ...
            <tfoot><tr>
                <td colspan="2" style="padding:14px 4px;text-align:right;font-weight:700">{{ __('commero::app.order_notifications.total_amount') }}</td>
                <td style="padding:14px 4px;text-align:right;font-weight:700">{{ number_format((float) $order->total_amount, 2, '.', ' ') }}</td>
            </tr></tfoot>
        </table>

        @if(filled($order->comment))<p style="margin:24px 0 0"><strong>{{ __('commero::app.order_notifications.comment') }}:</strong><br>{{ $order->comment }}</p>@endif
        @if(filled($adminUrl))
